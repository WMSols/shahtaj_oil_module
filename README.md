...
This repository contains **custom Odoo addons only**. Odoo 19 Community core, Python virtualenv, database dumps, and `odoo.conf` are **not** included — install those on each developer machine.

## What is in this repo

| Path | Purpose |
|------|---------|
| `shahtaj-web-portal/shahtaj_oil/` | Main application module (distributor portal, order booker, API, accounting hooks) |
| `shahtaj-web-portal/fastapi/` | OCA FastAPI integration (required dependency) |
| `shahtaj-web-portal/endpoint_route_handler/` | OCA route handler (required by `fastapi`) |
...
```
shahtaj-web-portal/shahtaj_oil/static/src/
├── js/
...
3. Clone this repo and add to `addons_path` in your local `odoo.conf`:
   ...
   ```ini
   addons_path = /path/to/odoo-19.0/addons,/path/to/this-repo/shahtaj-web-portal
   ```
...
- `odoo.conf` — contains DB passwords
